    ...
    useEffect(() => {
        if(navigator.geolocation){
            navigator.geolocation.getCurrentPosition(position => {
                    ...
                    switch(err.code){
                        case err.PERMISSION_DENIED : setIsLocationAllowed(false)
                                                     break;
                        default :  setIsLocationAllowed(false)
                                  break;                                           
                    }
                })
            }
                            ...
                            matches ?  
                                <Grid container spacing={0}>
                                        <StyledGrid item xs={12} lg={3} md={4} sm={12}>
                                            ...
                                            <Navigation />
                                            {
                                                    isLocationAllowed ?   <Switch>
                                                                                    <Route path='/' render={() => <FriendFinder lng={loc.lng} lat={loc.lat}/>} exact/>
                                            ...
                                            <Grid item lg={9} md={8}>
                                                    <Route path='/inbox/:userId' render={() => <Chat isDesktop = {true}/>}/>
                                            </Grid>            
                                  </Grid>    
                                      :
                                  <Grid style={{height : '100%'}} container spacing={0}>
                                        <Grid style={{height : '100%'}} item sm={12} xs={12}>
                                            <Switch>
                                                    <Route path='/inbox/:userId' render={() => <Chat isDesktop = {false}/>} />
                                                    <Route path='/' render={() => <MobileRoutes loc={loc} isLocationAllowed={isLocationAllowed}/>} />
                                            </Switch> 
                                        </Grid> 
                                  </Grid>
...
const MobileRoutes = props => {
    return <>
            <StyledGrid item xs={12} lg={12} md={12} sm={12}>
                ...
                <Navigation />
                {
                        props.isLocationAllowed ?   <Switch>
                                                        <Route path='/' render={() => <FriendFinder lng={props.loc.lng} lat={props.loc.lat}/>} exact/>
                                                        <Route path='/setting' component = {Setting} />
                                                        <Route path='/inbox' component = {Inbox} />
                                                </Switch> 
